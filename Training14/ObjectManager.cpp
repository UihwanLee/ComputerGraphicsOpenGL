...
void ObjectManager::SetChangeActive(int mode)
{
	...
	// Á¤À°¸éÃ¼¸¦ º¸ÀÌ°Ô ÇÏ°í Á¤»ç¸éÃ¼ ºñÈ°¼ºÈ­
	if (mode == 0)
	{
...
void ObjectManager::ShowCubeShapeface(int idx, int face)
{
	...
	if (face == 0)		start_idx = 0; // À­¸é 
	else if (face == 1)	start_idx = 6; // ¾Õ¸é
	else if (face == 2)	start_idx = 12; // ¿ìÃø¸é
	else if (face == 3)	start_idx = 18; // ÁÂÃø¸é
	else if (face == 4)	start_idx = 24; // ¹Ø¸é
	else if (face == 5)	start_idx = 30; // ¹Ø¸é

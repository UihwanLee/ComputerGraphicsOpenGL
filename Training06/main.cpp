...
vector<Rect> gParticleRect2;
int g_rect_num = 0;
int g_cur_rect = 0;
bool g_left_button = false;

bool isAnim = false;
bool isFourEight = false;
...
GLvoid GenerateParticleRect1(int idx)
{
	...
	// 사각형 파티클 이등분
	temp.color = gRect[idx].color;
	temp.pivot.x = gParticleRect[0].pivot.x - gParticleRect[0].RECT_SIZE / 2.0f;
	temp.pivot.y = gParticleRect[0].pivot.y;
	temp.RECT_SIZE = gParticleRect[0].RECT_SIZE - decreaseSIZE;
	temp.isActive = false;

	gParticleRect2.emplace_back(temp);
	...
	// 사각형 파티클 이등분
	temp.color = gRect[idx].color;
	temp.pivot.x = gParticleRect[1].pivot.x - gParticleRect[1].RECT_SIZE / 2.0f;
	temp.pivot.y = gParticleRect[1].pivot.y;
	temp.RECT_SIZE = gParticleRect[1].RECT_SIZE - decreaseSIZE;
	temp.isActive = false;
	...
	// 사각형 파티클 이등분
	temp.color = gRect[idx].color;
	temp.pivot.x = gParticleRect[2].pivot.x - gParticleRect[2].RECT_SIZE / 2.0f;
	temp.pivot.y = gParticleRect[2].pivot.y;
	temp.RECT_SIZE = gParticleRect[2].RECT_SIZE - decreaseSIZE;
	temp.isActive = false;
	...
	// 사각형 파티클 이등분
	temp.color = gRect[idx].color;
	temp.pivot.x = gParticleRect[3].pivot.x - gParticleRect[3].RECT_SIZE / 2.0f;
	temp.pivot.y = gParticleRect[3].pivot.y;
	temp.RECT_SIZE = gParticleRect[3].RECT_SIZE - decreaseSIZE;
	temp.isActive = false;
...
GLvoid FourEightDir()
{
	...
	for (int i = 0; i < gParticleRect.size(); i++)
	{
		// 크기 감소
		gParticleRect[i].RECT_SIZE = gParticleRect[i].RECT_SIZE - 0.001f;
		if (gParticleRect[0].pivot.x <= gRect[0].pivot.x - 0.2f)
		{
			isFourEight = true;
			break;
		}

		if (isFourEight == false)
		{
			if (i == 0) dirX = -1.0f, dirY = 1.0f;
			else if (i == 1) { dirX = -1.0f, dirY = -1.0f; }
			else if (i == 2) { dirX = 1.0f, dirY = 1.0f; }
	...
	if (isFourEight)
	{
		for (int i = 0; i < gParticleRect2.size(); i++)
		{
			gParticleRect2[i].isActive = true;
			// 크기 감소
			gParticleRect2[i].RECT_SIZE = gParticleRect2[i].RECT_SIZE - 0.001f;
			if (gParticleRect2[i].RECT_SIZE <= 0.0f)
			{
				gParticleRect2[i].isActive = false;
				isAnim = false;
				return;
			}
		}
